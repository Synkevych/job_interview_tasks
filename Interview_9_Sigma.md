...
- [Junior Ruby Developer - Sigma](#junior-ruby-developer---sigma)
  ...
  - [Работа с базами данных](#работа-с-базами-данных)
    ...
    - [Для чего используются индексы](#для-чего-используются-индексы)
...
- каждый объект характеризуется своим состоянием(внутренние переменные) и поведением(методы)

Объектно ориентированое програмирование - методология программирования основанная на предствлении программы в виде совокупности объектов, каждый из которых являются екземпляром определенного класса. Все есть объект, система состоит из совокупности объектов, в объектов есть состояние(поля) и поведение(методы).

**Инкапсуляция** - однин объект не может изменять состояние другого, для этого он должен использовать его поведением, или его методами. Пользователь класса или подсистемы должен видеть и использовать только инткрфейс(список декларируемых свойств и методов) и не вникать во внутреннюю реализацию. Только сам объект должен изменять свое состояние, никто из вне не должен это делать напрямую.

**Наследование** - возможность породжать один класс от другого с сохранением всех всойств и методов класса-передка(суперкласса) добавляя при необхожимости новые свойства и методы. Призвано отобразить такое свойство реального мира как иерархичность. Возможность порождать один класс от другого с сохранением всех свойств и методов класса-предка, добавляя при необхидомости новые свойства и методы. 

**Полиморфизм** классы-потомки могут изменять реализацию метода каласса-предка, сохраняя его сигнатуру(таким образом сохраняя неизмененным интерфейс класса предка). Даеть возможность обрабатывать классы потомков таким образом как будто они принадлежат классу предку.  Возможность обращатся с наследниками одного класса как будто это один и тот же класс но при этом поведение у них будет разное. Если у вас есть много if или swith стоит подумать об полиморфизме.

Композиция гибче наследования поэтому ее рекомендуется использовать, если это возможно.

Шаблоны проектирования GRASP - являются фундаментальными принципами распределения ответственности между классами, состоит из 9 элементов:
- Information Expert: информация должна обрабатываться там, где она содержится
- Creator: Создавать екземпляр класса должен тот который  будет его использовать
- Controller: Собирает все входнные данные из многопоточного пула в едином однопоточном контроллере через который осуществляется взаимодействие с Бизнес Логикой(БД)
- Low Coupling: Минимум связей между объектами, так их легче переиспользовать
- High Cohesion: В одном классе должны быть вещи логически друг с другом связаны
- Polymorphism
- Pure Fabrication
- Indirection
...
**Don't Repeat Yourself**(не повторяйся) - его переиспользование через наследование или рефакторинг. Буквально нужно уходть от дублирования кода и по возможности использовать абстрацию для общих вещей.

Если более чем двух в местах есть одинаковый блок кода вынесите его в отдельный метод.

### REST архитектура

Архитектурный стиль взаимодействия компонентов распределённого клиент-серверного приложения в сети. Представляет собой наборсогласованых ограничений, учитывемых при проектировании распределенной системы. Упрощает роутинг и построение API, в определенных случаях приводит к увеличению производительности.
...
### extand vs include vs prepent

**include** — часто называется примесью, делает методы модуля доступными экземпляру класса. Он берет все методы из другого модуля и включает их в текущий модуль. Является основным способом расширить классы другими модулями. 
...
**extend** — вызывает include для синглтон-класса объекта. Добавляет расширенный класс к предкам одноэлементного класса расширяющихся классов. Другими словами **extend** смешивает функциональность модулей с классом и делает эти методы доступными для самого класса.  
...
**prepend** вызывает Module.prepend_features для каждого параметра в обратном порядке, таким образом методы модуля будут первыми. 
...
- Модуль это набор констант и методов в нем нет наследования
- Модулей можна импортировать несколько, клас наследуеться только один
...
Процесс разделения данных по отдельным таблицам, устраняет избыточность данных тем самым позволяет избежать нарущения целосности данных, и аномали изменения. Ненормализированая таблица может содержать повторяющиеся столбцы. База данных считается нормализированной если к ней применяется 3 нормальная форма и выше<(всего их 6).  
Сводится к устранению недостатков структуры базы данных, приводящих к различным аномалиям и нарушениям целостности данных.

Первая нормальная форма предполагает что сохраненные данные на пересичении строк и столбцов должны прдставлять скалярное значение а таблицы не должны содержать повторяющихся строк. 

Вторая НФ - каждый столбец не являющийся ключем должен зависеть от первичного ключа. 

Третья НФ - каждый столбец не являющийся ключем должен зависеть только от первичного ключа. 

Выды нормализации и какие из них основные(часто используемые)

### PRIMARY and FOREIGN KEY

**Первичный ключ** -  один из потенциальных ключей отношения, выбранный в качестве основного ключа (или ключа по умолчанию). Если первичный ключ состоит из единственного атрибута, его называют простым ключом. Может быть составным или значение auto-increment. 
...
В реляционной базе данных запись о книге часто разделяется на несколько частей (или «нормализуется») и хранится в отдельных таблицах, отношения между которыми определяются ограничениями первичных и внешних ключей. Реляционная модель создана таким образом, чтобы обеспечить целостность ссылочных данных между таблицами в базе данных. Данные нормализованы для снижения избыточности и в целом оптимизированы для хранения.  
